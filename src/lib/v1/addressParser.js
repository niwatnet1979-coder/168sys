...
function findSplitPoint(text) {
    ...
    if (thaiLimitMatch) return thaiLimitMatch.index + thaiLimitMatch[0].length;

    const engLimitMatch = text.match(/.+?(?:Co\.,?\s*Ltd|Limited)/i);
    if (engLimitMatch) return engLimitMatch.index + engLimitMatch[0].length;

    // Fallback split before address elements
    const addressTrigger = text.match(/(?:เลขที่|No\.|\d+\/\d+)/i);
    if (addressTrigger) return addressTrigger.index;
