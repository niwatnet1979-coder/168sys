...
export const getSettings = async (): Promise<SettingsData | null> => {
    try {
        ...
        if (!session) {
            ...
            // return null // Allow public read if RLS permits, or handle gracefully
        }
        ...
        const systemOptions: { [key: string]: SystemOption[] } = {}
 ...
 * Save Shop Settings
 */
export const saveShopSettings = async (settingsData: any): Promise<{ success: boolean; error?: string }> => {
 ...
 */
export const syncSystemOptions = async (category: string, items: SystemOption[] | string[]): Promise<{ success: boolean; error?: string }> => {
    const { data: { session } } = await supabase.auth.getSession()
